...
def quotation_extract(file_path, output_root=None):
    ...
    def ocr_pages(pdf_file):
        all_lines_with_page = []
        page_line_map = {}
        pages_local = convert_from_path(str(pdf_file), dpi=300, poppler_path=POPPLER_PATH)
        for page_num, page in enumerate(pages_local, start=1):
            print(f"Processing page {page_num}...")
            text = pytesseract.image_to_string(page, config="--oem 3 --psm 6")
            lines = [line.strip() for line in text.split("\n") if line.strip()]
            page_line_map[page_num] = lines
            all_lines_with_page.extend((line, page_num) for line in lines)
        return all_lines_with_page, page_line_map
